...
if ($Help) {
    Write-Info "🌐 Local Development with VPS Database"
    Write-Info "======================================"
    Write-Info ""
...
# Test VPS connection
if ($TestConnection) {
    Write-Info "🔍 Testing VPS connections..."
    ...
    try {
        ...
        Write-Error "❌ SQL Server connection failed: $($_.Exception.Message)"
    }
    ...
    try {
        ...
        Write-Success "✅ VPS application is running (Status: $($response.StatusCode))"
    } catch {
        Write-Warning "⚠️ VPS application not accessible: $($_.Exception.Message)"
    }
}

# Run database migrations
if ($RunMigrations) {
    Write-Info "🗄️ Running database migrations on VPS..."
...
if ($RunApp) {
    Write-Info "🚀 Starting local application connected to VPS database..."
    ...
    Write-Info ""
    Write-Success "🌟 Application starting... Press Ctrl+C to stop"
    Write-Info ""
    ...
    try {
        ...
        Write-Error "❌ Application startup failed: $($_.Exception.Message)"
        exit 1
    } finally {
        Set-Location "../../../.."
    }
...
if (!$TestConnection -and !$RunApp -and !$RunMigrations) {
    ...
    Write-Info ""
    Write-Info "💡 Example: .\connect-to-vps.ps1 -VpsIp $VpsIp -TestConnection"
}
